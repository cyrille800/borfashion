...
$('[name="file"]').change(function(){
    ...
    // $("#cv").attr("src",$(this).val());
            ...
            $(".modif").click(function(){
             ...
             $("#pour_la_pomotion").hide();
             $("#pour_la_modification").show();
              var categorie="";
                id=$(this).attr("cni");
            ...
            $(".promotion").click(function(){
              $("#champs_cache").val($(this).attr("cni"));
              $("#ancien_prix").val($(this).attr("prix"));
             $("#pour_la_modification").hide(); 
              $("#pour_la_pomotion").show();  
              $("#lol").fadeIn();          
            })

            $("#promotion").click(function(){
                $.get("../../entities/promotion.php",{operation:"ajouter",date_fin:$('[name="date_fin_promotion"]').val(),id_produit:$("#champs_cache").val(),prix:$('[name="nouveau_prix"]').val()},function(data){
                    if(data=="ok"){
                      notification("success","operation effectuer");
                    }else{
...
$("#soum").click(function(){
    ...
    notification("error",data);    
        ...
        if(parseInt($("[name='prix']").val())>max_prix){
        max_prix=parseInt($("[name='prix']").val());
            let my_range = $("#range_03").data("ionRangeSlider");
            my_range.update({
                            max:max_prix,
                            to:max_prix
                        })
        ...
        if(parseInt($("[name='quantite']").val())>max_qte){
        max_qte=parseInt($("[name='quantite']").val());
            let my_ranges = $("#range_04").data("ionRangeSlider");
            my_ranges.update({
                            max:max_qte,
                            to:max_qte
                        })
...
$('[name="nouveau_prix"]').keyup(function(){
  var ancien=parseInt($('#ancien_prix').val());
  var nouveau=Math.round((parseInt($(this).val())*5)/ancien);
  if(nouveau==0){
    nouveau++;
  }
  $('.rating input[id="start1"]').prop('checked', false );
  $('.rating input[id="start2"]').prop('checked', false );
  $('.rating input[id="start3"]').prop('checked', false );
  $('.rating input[id="start5"]').prop('checked', false );
  $('.rating input[id="start4"]').prop('checked', false );
  $('.rating input[id="start'+nouveau+'"]').prop('checked', true );
})
